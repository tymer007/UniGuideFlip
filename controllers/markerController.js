...
export const updateMarker = async (req, res) => {
  ...
  if (!mongoose.Types.ObjectId.isValid(id)) {
    return res
      .status(404)
      .json({ success: false, message: "Invalid marker ID" });
  }
  ...
  try {
    ...
    if (marker.coverPhoto) {
      await cloudinary.uploader.destroy(
        marker.coverPhoto.split("/").pop().split(".")[0]
      );
    ...
    console.error(
      "Error in handling cover photo update:",
      error.message || error
    );
    return res
      .status(500)
      .json({ success: false, message: "Error updating cover photo" });
  }
...
export const deleteMarker = async (req, res) => {
  ...
  try {
    ...
    if (!marker) {
      return res
        .status(404)
        .json({ success: false, message: "Marker not found" });
    }
    ...
    res
      .status(200)
      .json({ success: true, message: "Marker deleted successfully" });
  } catch (error) {
